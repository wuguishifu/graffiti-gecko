export default class SimplexNoiseOctave {
  ...
  // Helpers
  static fastfloor(x: number) {
    return x > 0 ? x | 0 : (x | 0) - 1;
  }

  static dot3(g: { x: number; y: number; z: number }, x: number, y: number, z: number) {
    return g.x * x + g.y * y + g.z * z;
  }
  static dot4(g: { x: number; y: number; z: number; w: number }, x: number, y: number, z: number, w: number) {
  ...
  noise2D(xin: number, yin: number) {
    ...
    if (t0 >= 0) {
      t0 *= t0;
      n0 = t0 * t0 * SimplexNoiseOctave.dot3(SimplexNoiseOctave.grad3[gi0], x0, y0, 0);
    }
    let t1 = 0.5 - x1 * x1 - y1 * y1;
    if (t1 >= 0) {
      t1 *= t1;
      n1 = t1 * t1 * SimplexNoiseOctave.dot3(SimplexNoiseOctave.grad3[gi1], x1, y1, 0);
    }
    let t2 = 0.5 - x2 * x2 - y2 * y2;
    if (t2 >= 0) {
      t2 *= t2;
      n2 = t2 * t2 * SimplexNoiseOctave.dot3(SimplexNoiseOctave.grad3[gi2], x2, y2, 0);
    }
